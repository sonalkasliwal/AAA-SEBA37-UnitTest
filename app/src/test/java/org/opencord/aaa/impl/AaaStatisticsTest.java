...
public class AaaStatisticsTest extends AaaTestBase {
	...
	public void testAaaStatisticsForAcceptedPackets() throws Exception {
		...
		RADIUS successPacket = constructRadiusCodeAccessChallengePacket(RADIUS.RADIUS_CODE_ACCESS_ACCEPT, EAP.SUCCESS);
	...
	public void testAaaStatisticsForDefectivePackets() throws Exception {
		...
		RADIUS rejectedPacket = constructRadiusCodeAccessChallengePacket(RADIUS.RADIUS_CODE_ACCESS_REJECT, EAP.FAILURE);
		...
		// Calculated the toatl round trip time
	...
	class TestSocketBasedRadiusCommunicator extends SocketBasedRadiusCommunicator {
		...
		public void handlePacketFromServer(PacketContext context) {
			...
			try {
				if (null == context) {
				aaaManager.aaaStatisticsManager.handleRoundtripTime(incomingPkt.getIdentifier());
				aaaManager.handleRadiusPacket(incomingPkt);
					...
					aaaManager.aaaStatisticsManager.handleRoundtripTime(incomingPkt.getIdentifier());
					aaaManager.handleRadiusPacket(incomingPkt);
					incomingPkt = RADIUS.deserializer().deserialize(incomingPkt.generateAuthCode(), 0, 1);
				}
